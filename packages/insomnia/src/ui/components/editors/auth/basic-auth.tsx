import React, { FC } from 'react';

import { t } from '../../../../common/i18n';
import { AuthInputRow } from './components/auth-input-row';
import { AuthTableBody } from './components/auth-table-body';
import { AuthToggleRow } from './components/auth-toggle-row';

export const BasicAuth: FC<{ disabled?: boolean }> = ({ disabled = false }) => (
  <AuthTableBody>
    <AuthToggleRow label={t('auth.enabled')} property="disabled" invert disabled={disabled} />
    <AuthInputRow label={t('auth.username')} property="username" disabled={disabled} />
    <AuthInputRow label={t('auth.password')} property="password" mask disabled={disabled} />
    <AuthToggleRow
      label={t('auth.useIso88591')}
      help={t('auth.useIso88591Help')}
      property='useISO88591'
      disabled={disabled}
    />
  </AuthTableBody>
);
